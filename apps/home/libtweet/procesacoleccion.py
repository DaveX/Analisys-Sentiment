...
def procesa_vocabulario(text, c_stopwords, c_porter, c_lematizacion, c_alpha, c_lower, v):
	...
	words = [w for w in vocab if w.lower() not in stopwords]
	vocab = sorted(set(words))
	type(vocab)
	...
	words = [w for w in vocab if w.isalpha()]
	vocab = sorted(set(words))
	...
	words = [porter.stem(t) for t in vocab]
	vocab = sorted(set(words))
	...
	words=[wnl.lemmatize(t) for t in vocab]
	vocab = sorted(set(words))
...
while flag=='1':
	...
	print "Indique No(0) o Si(1) para cada opcion\n"
	a_stopwords = raw_input("\tEliminar Stopwords: ")
	a_porter = raw_input("\tAplica Porter: ")
	a_lematizacion = raw_input("\tAplica Lematizacion: ")
